...
val_path = Path(
    "/media/fumcomp/9CAA7029AA6FFDD8/fold_0/project/data/val")
...
for e in range(17,20):
    ...
    for img, label in tqdm(val_loader):
        ...
        with torch.no_grad():
            ...
            node3 = torch.tensor(img[3]).cuda()
            node4 = torch.tensor(img[4]).cuda()

            label0 = torch.tensor(label[0]).cuda()
            label1 = torch.tensor(label[1]).cuda()
            label2 = torch.tensor(label[2]).cuda()
            label3 = torch.tensor(label[3]).cuda()
            label4 = torch.tensor(label[3]).cuda()

            pred0, pred1, pred2, pred3, pred4 = model(node0, node1, node2, node3, node4)
            pred = pred0
