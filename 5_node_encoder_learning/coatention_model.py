...
class CoattentionModel(nn.Module):
    def __init__(self, num_classes=1, all_channel=256, all_dim=60, num_node=2):  # 473./8=60
        ...
        self.conv_fusion = nn.Conv2d(all_channel * num_node, all_channel, kernel_size=3, padding=1, bias=True)
    ...
    def forward(self, input1, input2, input3, input4, input5):  # Note input2 can be multi-frame image
        ...
        nodes1, x1_2, x1_1 = self.encoder(input1)
        nodes2, x2_2, x2_1 = self.encoder(input2)
        nodes3, x3_2, x3_1 = self.encoder(input3)
        nodes4, x4_2, x4_1 = self.encoder(input4)
        nodes5, x5_2, x5_1 = self.encoder(input5)
        ...
        x4s = torch.zeros(batch_num, 1, input_size[0], input_size[1]).cuda()
        x5s = torch.zeros(batch_num, 1, input_size[0], input_size[1]).cuda()
        # start_time = time.time()
        for ii in range(batch_num):
            node1 = nodes1[ii, :, :, :][None].contiguous().clone()
            node2 = nodes2[ii, :, :, :][None].contiguous().clone()
            node3 = nodes3[ii, :, :, :][None].contiguous().clone()
            node4 = nodes4[ii, :, :, :][None].contiguous().clone()
            node5 = nodes5[ii, :, :, :][None].contiguous().clone()
            # print('size:', query.size())
            for passing_round in range(self.propagate_layers):

                att1_1 = self.generate_attention(node1, node2)
                att1_2 = self.generate_attention(node1, node3)
                att1_3 = self.generate_attention(node1, node4)
                att1_4 = self.generate_attention(node1, node5)
                att1_cat=torch.cat([att1_1, att1_2, att1_3, att1_4], 1)
                attention1 = self.conv_fusion(att1_cat)  # message passing with concat operation

                attention2 = self.conv_fusion(torch.cat([self.generate_attention(node2, node1),
                                                         self.generate_attention(node2, node3),
                                                         self.generate_attention(node2, node4),
                                                         self.generate_attention(node2, node5)], 1))

                attention3 = self.conv_fusion(torch.cat([self.generate_attention(node3, node1),
                                                         self.generate_attention(node3, node2),
                                                         self.generate_attention(node3, node4),
                                                         self.generate_attention(node3, node5)], 1))

                attention4 = self.conv_fusion(torch.cat([self.generate_attention(node4, node1),
                                                         self.generate_attention(node4, node2),
                                                         self.generate_attention(node4, node3),
                                                         self.generate_attention(node4, node5)], 1))

                attention5 = self.conv_fusion(torch.cat([self.generate_attention(node5, node1),
                                                         self.generate_attention(node5, node2),
                                                         self.generate_attention(node5, node3),
                                                         self.generate_attention(node5, node4)], 1))

                h_v1 = self.ConvGRU(attention1, node1)
                # h_v1 = self.relu_m(h_v1)
                h_v2 = self.ConvGRU(attention2, node2)
                # h_v2 = self.relu_m(h_v2)
                h_v3 = self.ConvGRU(attention3, node3)
                # h_v3 = self.relu_m(h_v3)
                h_v4 = self.ConvGRU(attention4, node4)
                h_v5 = self.ConvGRU(attention5, node5)
                ...
                if passing_round == self.propagate_layers - 1:
                    x1s[ii, :, :, :] = self.my_fcn(h_v1, nodes1[ii, :, :, :][None].contiguous(), input_size,
                                                   x1_2[ii].unsqueeze(0),
                                                   x1_1[ii].unsqueeze(0))

                    x2s[ii, :, :, :] = self.my_fcn(h_v2, nodes2[ii, :, :, :][None].contiguous(), input_size,
                                                   x2_2[ii].unsqueeze(0),
                                                   x2_1[ii].unsqueeze(0))

                    x3s[ii, :, :, :] = self.my_fcn(h_v3, nodes3[ii, :, :, :][None].contiguous(), input_size,
                                                   x3_2[ii].unsqueeze(0),
                                                   x3_1[ii].unsqueeze(0))

                    x4s[ii, :, :, :] = self.my_fcn(h_v4, nodes4[ii, :, :, :][None].contiguous(), input_size,
                                                   x4_2[ii].unsqueeze(0),
                                                   x4_1[ii].unsqueeze(0))

                    x5s[ii, :, :, :] = self.my_fcn(h_v5, nodes5[ii, :, :, :][None].contiguous(), input_size,
                                                   x5_2[ii].unsqueeze(0),
                                                   x5_1[ii].unsqueeze(0))

        # end_time = time.time()
        # print('network fedforward time:', end_time-start_time)
        return x1s, x2s, x3s, x4s, x5s
