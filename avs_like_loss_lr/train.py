...
optimizer = torch.optim.SGD(
    [{'params': get_1x_lr_params(model), 'lr': 1 * 0.00025},  # Learn for specific layers, some layers don’t learn
     {'params': get_10x_lr_params(model), 'lr': 10 * 0.00025}],
    lr=0.00025, momentum=0.9, weight_decay=0.0005)
loss_calc2 = torch.nn.BCELoss()
logger = open('./logs/logs.txt', 'a')
resume = 0
if resume:
    state_model = torch.load(
        '/home/fumcomp/Desktop/poorsoltani/avs_like_loss_lr/epoch_' + str(resume - 1) + '.pth')
...
for i in range(resume, EPOCHE):
    ...
    for step, (img, label) in tqdm(enumerate(train_loader), total=(len(train_loader.batch_sampler))):
        ...
        loss2_0 = loss_calc2(pred0, label0)
        loss1_0 = loss_calc1(pred0, label0)
        loss0 = loss2_0 if loss1_0 == -1 else (loss1_0 + 0.8 * loss2_0)

        loss2_1 = loss_calc2(pred1, label1)
        loss1_1 = loss_calc1(pred1, label1)
        loss1 = loss2_1 if loss1_1 == -1 else (loss1_1 + 0.8 * loss2_1)

        loss2_2 = loss_calc2(pred2, label2)
        loss1_2 = loss_calc1(pred2, label2)
        loss2 = loss2_2 if loss1_2 == -1 else (loss1_2 + 0.8 * loss2_2)

        # class_balanced_cross_entropy_loss(pred, labels, size_average=False)
    ...
    }, '/home/fumcomp/Desktop/poorsoltani/avs_like_loss_lr/epoch_' + str(i) + '.pth')
    print('##########################################')
