        ...
        for ii in range(batch_num):
            ...
            for passing_round in range(self.propagate_layers):
                ...
                if passing_round == self.propagate_layers - 1:
                    x1s[ii, :, :, :] = self.my_fcn(h_v1, nodes0[ii, :, :, :][None].contiguous(), input_size,x2_0[ii].unsqueeze(0),
                                                   x1_0[ii].unsqueeze(0))
                    x2s[ii, :, :, :] = self.my_fcn(h_v2, nodes1[ii, :, :, :][None].contiguous(), input_size,x2_1[ii].unsqueeze(0),
                                                   x1_1[ii].unsqueeze(0))
                    x3s[ii, :, :, :] = self.my_fcn(h_v3, nodes2[ii, :, :, :][None].contiguous(), input_size,x2_2[ii].unsqueeze(0),
                                                   x1_2[ii].unsqueeze(0))
