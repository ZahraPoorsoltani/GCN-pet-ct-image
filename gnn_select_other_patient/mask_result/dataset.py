...
class LungDataset(torch.utils.data.Dataset):
    def __init__(self, root, train_root, augment_params, num_nodes=3):
        self.all_files = self.extract_files(root)
        self.train_files = self.extract_train_files(train_root)
    ...
    def extract_train_files(self, root):
        """
        Extract the paths to all slices given the root path (ends with train or val)
        """
        files = []
    ...
    def get_other_patient_idx(self, target_pname, target_range_idx, patient2=''):
        search_idx = np.random.randint(len(self.train_files))
        patient_name = self.train_files[search_idx].parts[-3]
        if patient2:
        ...
        patient_range = self.train_Index[patient_name]
    ...
    def __getitem__(self, idx):
        ...
        for j,i in enumerate([idx, idx2, idx3]):
            if j==0:
                file_path = self.all_files[i]
            else:
                file_path = self.train_files[i]
