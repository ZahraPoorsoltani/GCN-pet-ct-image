...
class CoattentionModel(nn.Module):
    def __init__(self, num_classes=1, all_channel=256, all_dim=60):  # 473./8=60
        ...
        self.conv_fusion_pet = nn.Conv2d(all_channel * 2, all_channel, kernel_size=3, padding=1, bias=True)
    ...
    def forward(self, input1, input2, input3):  # Note input2 can be multi-frame image
        ...
        for ii in range(batch_num):
            ...
            node0 = self.conv_fusion_pet(torch.cat([node0, pet0], 1))
            node1 = self.conv_fusion_pet(torch.cat([node1, pet1], 1))
            node2 = self.conv_fusion_pet(torch.cat([node2, pet2], 1))
