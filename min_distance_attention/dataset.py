...
class LungDataset(torch.utils.data.Dataset):
    def __init__(self, root, augment_params, num_nodes=5, eval=False):
        ...
        weights = torch.load('/media/fumcomp/9CAA7029AA6FFDD8/fold_0/project/epoch_40.pth', map_location='cuda')[
            'model']
        ...
        self.center_contours = pickle.load(
            open(
                '/media/fumcomp/9CAA7029AA6FFDD8/fold_0/project/todo/min_distance_neighbouring/patient_train_center',
                'rb'))
    ...
    def predict_mask(self, data):
        with torch.no_grad():
            ...
            output = torch.sigmoid(self.unet(inpt).squeeze())
            output = output > 0.5
    ...
    def min_neighbours(self, target_mask, num_neighbours):
        ...
        for cont in ref_contours:
            ...
            min_dist = 1000
            for other_mask in self.center_contours:
        ...
        min_dist_cont = np.argmin(arr_dist)
        return cal_dist[min_dist_cont]
    ...
    def __getitem__(self, idx_base):
        ...
        if np.any(target_pred_mask):
            for i in range(self.num_nodes - 1):
                self.min_neighbours(target_pred_mask,3)
